...
If you're generating a single file and want to use redo-stamp on
the whole file, the naive approach is to do this:

	redo-ifchange foo.c
	gcc foo.c -o $3
	cat $3 > redo-stamp

However, you can have the file be stamped as it's generated by
using the 'tee' utility and the temporary file syntax available
in many modern shells (resulting in a faster build):

	redo-ifchange foo.c
	gcc foo.c -o /dev/stdout | tee >(redo-stamp)

Note that this will not work:

	redo-ifchange foo.c
    gcc foo.c -o >(tee >(redo-stamp))

gcc and other utilities specifically detect being redirected to stdout,
and avoid doing seeks when this is the case. When you use process
substitution to send gcc's output to tee, it doesn't realize the file
descriptor is a pipe, and will try to do seeks on it, resulting in
'Illegal Seek' errors.
